...
class ProgramPointChildrenEditor extends Component
{
    ...
    public $children = [];    public $showModal = false;
    ...
    {
        $points = EventTemplateProgramPoint::with(['currency', 'tags'])
            ->where('id', '!=', $this->programPoint->id)
            ->whereNotIn('id', $this->programPoint->children->pluck('id'))
            ->orderBy('name')
            ->get();
    ...
    }    public function applyFilters()
    {
        ...
        if (!empty($this->searchTerm) && strlen($this->searchTerm) > 100) {
            $this->searchTerm = substr($this->searchTerm, 0, 100);
            $this->logSecurityEvent('search_term_truncated', 'Search term exceeded 100 characters');
        }
    ...
    }public function loadChildren()
    {
        ...
        $this->children = $children->map(function ($child) {
            return [
                ...
                'group_size' => $child->group_size,                'currency' => $child->currency ? $child->currency->toArray() : null,
    ...
    public function render()
    {
        ...
        return view('livewire.program-point-children-editor-simple', [
            ...
            'availablePoints' => collect($this->availablePoints),
            'filteredPoints' => collect($this->filteredPoints),
            'allTags' => $allTags,

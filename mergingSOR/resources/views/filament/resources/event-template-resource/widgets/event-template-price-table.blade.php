        ...
        <table class="min-w-full bg-white border border-gray-200">
            <thead>
                <tr class="bg-gray-50">
                    ...
                    <th class="px-4 py-2 border-b text-right">PLN</th>
                    @php
                        $allCurrencies = collect($this->detailedCalculations)->flatMap(function($currencies) {
                            return array_keys($currencies);
                        })->unique()->filter(fn($c) => $c !== 'PLN')->values();
                    @endphp
                    @foreach($allCurrencies as $currencyCode)
                        <th class="px-4 py-2 border-b text-right">{{ $currencyCode }}</th>
                    @endforeach
                </tr>
            </thead>
            <tbody>
                @foreach($this->detailedCalculations as $qty => $currencies)
                    <tr>
                        <td class="px-4 py-2 border-b">{{ $qty }} osób</td>
                        <td class="px-4 py-2 border-b text-right font-semibold">
                            @if(isset($currencies['PLN']) && isset($currencies['PLN']['total']) && $currencies['PLN']['total'] > 0)
                                {{ number_format($currencies['PLN']['total'] / $qty, 2) }} PLN
                            @else
                                -
                            @endif
                        </td>
                        @foreach($allCurrencies as $currencyCode)
                            <td class="px-4 py-2 border-b text-right font-semibold">
                                @if(isset($currencies[$currencyCode]) && isset($currencies[$currencyCode]['total']) && $currencies[$currencyCode]['total'] > 0)
                                    {{ number_format($currencies[$currencyCode]['total'] / $qty, 2) }} {{ $currencies[$currencyCode]['points'][0]['currency_symbol'] ?? $currencyCode }}
                                @else
                                    -
                                @endif
                            </td>
